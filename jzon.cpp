...
static int parse_table(const char** input, JzonValue* output, bool root_object)
{
    if (current(input) == '{')
        next(input);
    else if (!root_object)
...
JzonParseResult jzon_parse(const char* input)
{
    ...
    result.output = (JzonValue*)zalloc_zero(sizeof(JzonValue));
    skip_whitespace(&input);
...
JzonValue* jzon_get(JzonValue* table, const char* key)
{
    ...
    while (first <= last)
    {
        if (table->table_value[middle]->key_hash < key_hash)
            first = middle + 1;
        else if (table->table_value[middle]->key_hash == key_hash)
            return table->table_value[middle]->value;
        else
            last = middle - 1;
